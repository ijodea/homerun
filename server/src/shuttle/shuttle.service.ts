...
export class ShuttleService {
  ...
  getMStationTime(currentTime: Date): number | null {
    const MJUtoMS = this.parseCsvFile(
      path.join(__dirname, '../timetable/mStation.csv'),
      2,
    );
  ...
  getEverlineTime(
    estimatedTravelTimeToStation: number,
    currentTime: Date,
  ): number | null {
    const MStoGS = this.parseCsvFile(
      path.join(__dirname, '../timetable/everline.csv'),
      0,
    );
  ...
  getAllTransportOptions(currentTime: Date): {
    station: string;
    ...
    return [
      {
        station: 'GStation Shuttle',
        arrivalTime: gStationArrival,
        cost: 0,
        remainingSeats: null,
      },
      {
        station: 'MStation Shuttle',
        arrivalTime: mStationArrival,
        cost: 0,
        remainingSeats: null,
      },
      {
        station: 'Everline Subway',
        arrivalTime: everlineArrival,
        cost: 1450,
        remainingSeats: null,
      },
    ];
