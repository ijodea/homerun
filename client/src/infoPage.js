...
const Card = styled.div`
  background: white;
  border: 6px solid
    ${(props) => (props.type === "shuttle" ? "#001C4A" : "#C00305")};
  border-radius: 8px;
  padding: 20px;
  margin: 20px;
...
const Title = styled.h2`
  ...
  background-color: ${(props) =>
    props.type === "shuttle" ? "#001C4A" : "#C00305"};
  padding: 10px;
  border-radius: 4px;
  font-size: 1.5em;
  margin-bottom: 10px;
...
const Info = () => {
  ...
  const fetchBusInfo = async () => {
    try {
      const response = await fetch(`http://localhost:8000/api/bus/${direction}`);
  ...
  const fetchShuttleInfo = async () => {
    try {
      const response = await fetch(
        `http://localhost:8000/api/shuttle/${direction}`
      );
          ...
          {sortedInfo.map((info, index) => (
            ...
            >
              <Card type={info.type}>
                ...
                <div>
                  <InfoItem>
                    <strong>출발 시간:</strong> {info.departureTime}
                  </InfoItem>
                  <InfoItem>
