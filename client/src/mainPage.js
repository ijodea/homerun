...
const MainPage = () => {
    ...
    const fetchEfficientTransport = async () => {
        try {
            const busResponse = await fetch(`http://localhost:8000/api/bus/mju-to-giheung`);
            const shuttleResponse = await fetch(`http://localhost:8000/api/shuttle/next`);
